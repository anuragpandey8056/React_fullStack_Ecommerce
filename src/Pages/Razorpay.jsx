...
const Razorpay = ({ customerData }) => {
    ...
    const JSON_SERVER_URL = "http://localhost:3000"; // Make sure this matches your JSON server port
    ...
    const handleRazorpayPayment = async () => {
      ...
      try {
        ...
        try {
          const orderResponse = await axios.post(`${JSON_SERVER_URL}/orders`, orderData);
        ...
        const options = {
          ...
          handler: function (response) {
            ...
            const paymentData = {
              ...
              amount: amountInPaise,
              status: "success",
              payment_method: response.razorpay_payment_method || "unknown",
              created_at: new Date().toISOString(),
              customer_details: customerData || {}
            };
            
            axios.post(`${JSON_SERVER_URL}/payments`, paymentData)
              .then(() => {
          ...
          config: {
            display: {
              // Configure the preferred payment methods order
              // UPI is placed first in the list to make it more prominent
              preferences: {
